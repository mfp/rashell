...
let _run exec detach ?add_host ?cap_add ?cap_drop ?env ?device ?entrypoint ?expose ?hostname ?link ?memory ?publish ?tty ?user ?uid ?privileged ?restart ?argv image =
  let open Printf in
  let dockerargv =
    maybe_concat [
      Some([|
...
let run =
  _run exec_utility true

let run_utility =
  _run exec_utility false

let run_query =
  _run exec_query false

let run_test =
  _run exec_test false

let run_shell =
  _run exec_shell false
